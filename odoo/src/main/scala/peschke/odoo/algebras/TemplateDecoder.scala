...
object TemplateDecoder      {
  ...
  def default[F[_]: JsonLoader: MonadThrow: LocationNameExpander](datesGenerator: DatesGenerator): TemplateDecoder[F] =
    new TemplateDecoder[F] {
      ...
      private def mkPickingDecoder
        ...
        : Decoder[F[List[PickingTemplate]]] = {
        ...
        accumulatingDecoder { c =>
          ...
          (
            ...
            case (frequency, timeOfDay, pickingName, moveType, pickingTypeId, partnerId, disabled, moves) =>
              val result = for {
                date <- datesGenerator.generate(frequency)
                dow = DayOfWeek.ofDay(date)
                tod  <- timeOfDay.expanded.toList
              } yield {
      ...
      private def mkEntryDecoder(implicit pickingDecoder: Decoder[F[List[PickingTemplate]]]): Decoder[F[Entry]] =
        accumulatingDecoder { c =>
          (
            c.downField("label").asAcc[EntryLabel],
            c.downField("tags").asAcc[List[Tag]],
            c.downField("pickings").asAcc[NonEmptyList[F[List[PickingTemplate]]]].map(_.toList.flatSequence)
      ...
      override def decode(templateSource: JsonLoader.Source, knownIdsOpt: Option[JsonLoader.Source]): F[Template] =
        knownIdsOpt.fold(KnownIds.empty.pure[F])(loadKnownMappings).flatMap { knownIds =>
          ...
          implicit val pickingDecoder: Decoder[F[List[PickingTemplate]]] = mkPickingDecoder(knownIds)
