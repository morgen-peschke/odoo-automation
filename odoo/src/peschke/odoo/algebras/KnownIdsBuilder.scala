...
object KnownIdsBuilder      {
  ...
  def default[F[_]: ServiceCallBuilder: JsonRpc: MonadThrow: LoggerFactory]: KnownIdsBuilder[F] =
    new KnownIdsBuilder[F] {
      ...
      private def buildRequest(model: ModelName, nameField: FieldName): F[RpcServiceCall] =
        ServiceCallBuilder[F].fromAction(
          Search(
            model = model,
      ...
      private def load[Id: Decoder]
        (model: ModelName, fieldName: FieldName)
        : F[ValidatedNel[DecodingFailure, Map[String, Id]]] =
