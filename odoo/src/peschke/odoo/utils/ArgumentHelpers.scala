...
object ArgumentHelpers {
    ...
    Argument.from(names.mkString("|")) { raw =>
      E.withNameInsensitiveOption(raw)
        .orElse {
          ...
          uniquePrefixes
            .find {
              case (prefix, _) => lowerRaw.startsWith(prefix)
            }
            ._2F
        }
  ...
  private[utils] def calculateUnambiguousPrefixes[E <: enumeratum.EnumEntry](implicit E: enumeratum.Enum[E])
  : Vector[(String, E)] = {
    @tailrec
    def loop(current: Chain[(Chain[Char], (Chain[Char], E))],
             accum: Chain[(Chain[Char], E)]): Vector[(String, E)] = {
      val (canGrowNoLonger, grown) =
        current
          .map { case (prefix, (suffix, value)) =>
            suffix
              .uncons
      ...
      if (prefixes.length =!= uniques.length && grown.nonEmpty) loop(grown, ready)
        ...
        ready.concat(grown.map { case (prefix, (_, value)) => prefix -> value })
